...
- Introduced typed `ics_core::Error` (ADR-017 §error type relationship). `parse_events`, `parse_indices`, `insert_event`, `remove_event_by_summary`, `remove_events_by_indices` now return `Result<T, ics_core::Error>` instead of `Result<T, String>`. The `Parse` variant carries `message`, optional `line`, and optional `property` fields; current flat parser leaves the optional fields `None` and ADR-019's lexer-based parser will populate them. makeholiday/commands.rs bridges via `.map_err(|e| e.to_string())` temporarily; the bridge is removed when `MhError` lands in the next migration commit.
