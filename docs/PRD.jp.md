...
`makeholiday` が一切やらない事項。CalDAV / クラウドサービス同期は **この一覧には含めない** — v0.2.0 で導入予定（[§9 Roadmap](#9-roadmap) 参照）。
...
- マシン間でのカレンダー状態クラウド同期。 → CalDAV と一緒に v0.2.0 でスコープインする予定（[§9 Roadmap](#9-roadmap) 参照）。
...
### v0.2.0 — CalDAV / クラウドバックエンド（次戦線）

v0.2.0 シリーズは `makeholiday` を multi-backend カレンダークライアントへ拡張する。`ics-core` のパーサと型モデルは無改修で継承できる（CalDAV の応答もすべて構文的に正当な `VCALENDAR` ブロブだから）。作業は I/O 境界に集中し、型モデルは触らない。
...
- TUI ローンチ（スコープは [ADR-022](design/022-tui-front-end-policy.md) で確定済み、着手トリガはメンテナ判断 — [§8](#8-未決事項-open-questions) 参照）。
