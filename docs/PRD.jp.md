...
- **再利用可能 ICS ハンドリングライブラリ（crate 分離）.** `src/ics.rs` と型付き拡張モデルを独立 crate に切り出す。CLI はその上の薄い層に。型シェイプは [ADR-001](design/001-vendor-extension-typing.md) で確定済み、分離タイミングは将来の crate 分離 ADR が扱う。
