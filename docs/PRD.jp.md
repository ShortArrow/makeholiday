...
- **`ics-core` を別リポジトリに切り出し**、crates.io に公開。`ics-core` の version 契約はここから始まる（v0.1.x の monorepo 内実戦経験を踏まえる）。
- **`lazyics` — 対話的 TUI エディタ**: `.ics` ファイル向け、`lazygit` インスパイア。lazy- プレフィックスを TUI ツール命名規則として採用。独立バイナリにするか `makeholiday` CLI のサブコマンドとして同梱するかは、プロジェクト着手時に決定（元々 `makeholiday-tui` として計画されていた [ADR-022](design/022-tui-front-end-policy.md) を改訂）。
- **`icslint` — ICS リンタ**: `ics-core` を消費し、ベンダープレフィックス警告（"これは Microsoft 固有プロパティで Google クライアントは無視します" 等）と RFC コンプライアンスヒントを出す。初期ルールセットは着手時に決定、構造は既存 Rust リンタの慣例に従う。
