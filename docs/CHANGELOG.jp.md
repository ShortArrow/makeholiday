...
- **(破壊的変更)** `ics_core::profile::microsoft` モジュールを導入し、`X-MICROSOFT-CDO-BUSYSTATUS` をベンダーバンドルへ移動（ADR-001 Migration Step 4）。`VEvent.busystatus: BusyStatus` フィールドは **削除**、`BusyStatus` は `MsBusyStatus` に改名され `ics_core::microsoft` に移動。`VEvent.microsoft: Option<microsoft::EventExtensions>` が `busystatus: Option<MsBusyStatus>` を保持。パーサは `X-MICROSOFT-CDO-BUSYSTATUS:` をバンドル側へルーティング、フォーマッタはバンドルに値がある場合のみ `X-MICROSOFT-CDO-BUSYSTATUS:` を出力し、`TRANSP` のフォールバックもこの値から派生。Microsoft データもタイプ付き `transp` も無いイベントは TRANSP/BUSYSTATUS の両方を省略するようになった（以前は常にデフォルトから TRANSP:TRANSPARENT + BUSYSTATUS:FREE を出力していた）。CLI の `--busystatus` フラグは引き続き `free` がデフォルトのため、`makeholiday add` のユーザ可視挙動は不変。変わるのは内部表現とライブラリ API のみ。イベントの JSON 出力で `busystatus` は `microsoft` 配下にネスト。`microsoft::PREFIXES` は `X-MICROSOFT-CDO-` と `X-MICROSOFT-` を登録し、Step 6 で実装される prefix ルーティングに備える。
